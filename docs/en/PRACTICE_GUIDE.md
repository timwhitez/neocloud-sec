...
### Treat network-policy migration as a scoped, reversible change

A provider feature retirement or a recommended CNI change is a migration trigger for the services that actually use the affected component—not a reason to standardize every cloud on one network stack. Before rolling out default-deny policy, record OS/CNI compatibility, DNS and service dependencies, and paired allow/deny tests; preserve out-of-band and recovery access throughout the change and keep a tested rollback path. Kubernetes NetworkPolicy does not replace fabric, DPU or storage isolation boundaries.

### Eliminate static workload credentials

Use workload identity, scoped short-lived certificates/tokens, audience restriction, tenant/resource scope, revocation and, where justified, attested state. Revocation must reach credentials derived or generated by controllers and operators, not only the orchestrator-level grant. Metadata and default service identities must not grant broad provider/project authority to tenant workloads.
