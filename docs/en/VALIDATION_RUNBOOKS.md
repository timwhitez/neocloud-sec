...
For NCS-IAM-04, NCS-KMS-02 and NCS-ORC-04, also follow revocation through the operator into the backend: revoke a permitted cross-namespace association between synthetic resources, observe reconciliation and credential removal/rotation, and verify that the old disposable backend credential is denied while an independently authorized identity still succeeds [S9]. A Kubernetes denial alone does not revoke an existing database credential. Inspect actual service-account grants and every enabled authorization path; controller selectors and local caches are not substitutes for resource-side authorization [S10]. Retain redacted decision/request IDs, not credential values.

**Accept/evidence:** Tenant context survives every controller transition; unauthorized requests have no target-side effect; partial allocation is rolled back or quarantined. Save redacted policy exports, both request traces and actual-state reconciliation. Abort on foreign data visibility or unexpected privilege. Containment, credential rotation where needed and independent retest precede reopening.
...
For NCS-ASM-01, NCS-ORC-04 and NCS-CMP-03, distinguish upstream fix, distribution backport, provider image publication and actual node replacement. Record image/build identity and replacement or boot evidence, then verify workload completion, policy enforcement and recovery. A changed image-family pointer does not update existing nodes [S11]. Hotpatch enrollment does not remove a bulletin's reboot requirement; check the specific update rather than inferring its behavior from a standing calendar [S12]. Provider-only evidence remains a provider responsibility.

Treat a rootless feature gate, configured user namespace and actually rootless node as different states. Evaluate supported CNI/CSI/device/GPU combinations and recovery before an optional migration; do not mandate a beta feature or treat it as protection against every kernel flaw [S17].
...
Correlate policy changes, operator reconciliation, credential lifecycle, backend denials and storage decisions using tenant-safe IDs. For each provider audit source, record supported services/actions, enablement, delivery and retention; verify a harmless expected event arrives and a missing source is detected. An offered audit feature or successful collector exit does not prove coverage [S14]. Never export real tenant records to this repository.
...
For NCS-API-01, NCS-DAT-04 and NCS-ORC-02, review PV-creation authority, active deletion options, CSI identity, filesystem/access-point ownership and cloud resource policy together [S15]. Prefer patched vendor software and least privilege. Validate ownership with inert unit fixtures or vendor-supported non-destructive checks; never construct destructive volume handles or delete real data. Verify that authorized storage use survives the policy change. An affected driver is not automatically a storage-service breach; disabled affected configuration is a scoped applicability assertion requiring evidence and revalidation.
...
**Exercise:** Reject unapproved executable serialization and unauthorized artifact sources. Verify that a valid signature from an unapproved signer is not treated as safety. For NCS-SSC-02 and NCS-DAT-03, bind the actual artifact digest to an approved signer/builder, canonical source repository, build type and expected external parameters [S16]. Signature validity alone is insufficient; unknown or unauthorized inputs need an explicit decision rather than silent acceptance. Revoke a test artifact and verify registry, deployment, renderer and cache invalidation. Keep the controls applicable to the actual format: scanning is not proof that arbitrary model code is safe. Retain loader isolation rather than introducing a mandatory agent framework.
...
For NCS-AIR-03, enforce read-only access through the database/resource identity's actual privileges, not merely a tool description, SQL blocklist or default transaction setting [S19]. With disposable records and identities, verify an allowed read and a denied unauthorized side effect at the backend. Review inherited role memberships and executable functions, retain redacted decisions and follow RB-01 for credential revocation. Apply vendor fixes as well; least privilege does not establish that vulnerable software is remediated.
...
Vendor behavior is version/edition dependent. These sources support specific mechanisms, not all recommendations in a runbook. The [reference register](../../REFERENCES.md) records scoped dates and retrieval limitations; the [earlier source review](../../reviews/2026-09-05-evidence-followup.md) retains unresolved external-framework differences. Current advisories trigger applicability review, not automatic conformance or a mandatory platform migration.
