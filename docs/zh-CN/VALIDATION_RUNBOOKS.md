...
**准备：** 创建 A/B 模拟对象、卷和快照，记录 CSI／控制器身份、KMS 归属、不可变备份保留期和合同删除范围。联合审查 PV 创建权限、实际启用的删除选项、CSI 身份、Filesystem/Access Point 所有权和云资源策略，控制器优先采用厂商修复版本和最小权限。
...
**准备：** 使用模拟的不支持格式／无效输入、隔离加载器，不授予生产凭据。记录可接受格式、反序列化权限、制品摘要、来源、签名策略和运行边界。制品准入须将摘要绑定到批准的签名者/构建者身份、源码仓库和预期构建输入；仅签名有效并不足够 [S9]。
...
- [S9 — SLSA v1.2 制品验证](https://slsa.dev/spec/v1.2/verifying-artifacts)
